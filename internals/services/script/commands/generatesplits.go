...
func (s *GenerateScript) Handle(parameters Parameters) (string, int64, error) {
	...
	uploadedFiles, err := s.getUploads(fetchedChapters)
	if err != nil {
		return "", 0, err
	}
...
func (s *GenerateScript) getUploads(chapters []book.Chapter) ([]ai.UploadedFile, error) {
	...
	pages, err := s.book.GetPages(chapterIds)
	if err != nil {
		return nil, err
	}
	...
	uploadedFiles, err := s.ai.UploadFiles(files)
	if err != nil {
		return nil, err
	}
	...
	for i, r := range results {
		...
		err = s.book.UpdatePage(newPage.Id, &newPage)
		if err != nil {
			return nil, err
		}
